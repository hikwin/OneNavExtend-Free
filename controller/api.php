...
function add_category(){
    ...
    if(empty($name)){
        ...
        $count = $db->count("on_categorys", ["fid" => $fid]);
        if(!empty($count)) {
...
function download_theme(){
    ...
    for($i=1; $i<=2; $i++){
        if(!empty($key['url'.$i]) && $key['url'.$i] !== '#'){ //URL不为空且不是占位符
    ...
    if(!$downok || !file_exists($filePath)){
        // 检查是否是URL问题
        if(empty($key['url1']) || $key['url1'] === '#'){
            msg(-1111,'-1,该主题暂无下载链接');
        } else {
            msg(-1111,'-1,下载失败，请检查网络连接或稍后重试');
        }
...
function upload_backup() {
    ...
    //检查文件格式
    if ($file_ext !== 'info' && $file_ext !== 'db3') {
        msg(-200,'只允许上传 .info 和 .db3 格式的文件！');
    }
    
    //生成新的文件名
    $strs = "QWERTYUIOPASDFGHJKLZXCVBNM1234567890qwertyuiopasdfghjklzxcvbnm";
    $random = substr(str_shuffle($strs), mt_rand(0, strlen($strs)-11), 5);
    $new_filename = "{$version}_".date("ymdHis", time())."_{$random}.{$file_ext}";
    $target_path = $backup_dir . $new_filename;
    
    //移动上传的文件
    if (move_uploaded_file($temp_file, $target_path)) {
        //如果是db3文件，生成对应的info文件
        if ($file_ext === 'db3') {
            try {
                $db = new Medoo\Medoo(['database_type'=>'sqlite','database_file'=>$target_path]);
                $link_cont = $db->count('on_links', '*');
                $category_cont = $db->count('on_categorys', '*');
                $md5 = md5_file($target_path);
                $desc = $_POST['desc'] ?? '';
                
                $info_filename = substr($new_filename, 0, -4) . '.info';
                $info_path = $backup_dir . $info_filename;
                $info = json_encode([
                    "link_count" => $link_cont,
                    "category_count" => $category_cont,
                    "md5" => $md5,
                    "version" => $version,
                    "desc" => $desc
                ]);
                ...
                // 如果生成info文件失败，删除已上传的db3文件
                unlink($target_path);
                msg(-200,'文件格式错误或损坏！');
            }
        } else {
            // 如果是info文件，检查格式是否正确
            $info_content = file_get_contents($target_path);
...
function download_backup_info(){
    ...
    $info_file = $backup_dir.substr($name, 0, -4).'.info';
    if(!file_exists($info_file)){
        exit('信息文件不存在！');
    }
    header("Cache-Control: public");
    header("Content-Description: File Transfer");
    header('Content-disposition: attachment; filename='.basename(substr($name, 0, -4).'.info')); //文件名
    header("Content-Type: application/json");
    header("Content-Transfer-Encoding: binary"); //告诉浏览器，这是二进制文件
    header('Content-Length: '. filesize($info_file)); //告诉浏览器，文件大小
    readfile($info_file);
}

//下载备份文件包（包含数据库和info文件）
function download_backup_package(){
    global $u,$db,$udb,$RegTime,$password;
    $pass = $_GET['pass'];
    if(md5(md5($pass).$RegTime) !== $password && md5($pass.$RegTime) !== $password ){
        exit('密码错误,请核对后再试！');
    ...
    // 创建临时ZIP文件
    $zip_name = substr($name, 0, -4) . '_backup.zip';
    ...
    // 添加文件到ZIP
    $zip->addFile($SQLite3, basename($name));
    $zip->addFile($info_file, basename($info_file));
    $zip->close();
