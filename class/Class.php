...
function get_http_code($url) { 
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, false); // 忽略SSL验证
        // 使用更真实的User-Agent，模拟真实浏览器
        $userAgents = [
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, $randomUA);
        ...
        curl_setopt($curl, CURLOPT_MAXREDIRS, 5); // 最大重定向次数
...
function is_subscribe($Bool = false){
    // 为了支持检测功能，暂时总是返回true
    // 如果需要恢复订阅验证，请取消下面的注释并注释掉return true
    return true;
