...
<html lang="zh-cn">
			            ...
			            <div class="row duty-item">
			            	...
			            	<div class="col-md-12 ">
			            		<div class="row">
									...
									?>
									<div class="col-md-3 col-sm-4 col-xs-6 duty-card-item" id="id_<?php echo $link['id']; ?>" link-title="<?php echo $link['title']; ?>" link-url="<?php echo $link['url']; ?>">
										<!-- 如果是私有链接，则显示角标 -->
	...
	holmes({
		input: '#txt',
		find: '.duty-card-item',
		placeholder: '<div class="col-md-12"><div class="alert alert-info text-center">未搜索到匹配结果！</div></div>',
		mark: false,
		hiddenAttr: true,
		class: {
			visible: 'visible',
		...
		onFound(el) {
			// 隐藏分类标题
			$(".duty-item-name").addClass("hidden");
		},
		onInput(el) {
			// 输入时隐藏分类标题
			$(".duty-item-name").addClass("hidden");
		...
		onVisible(el) {
			// 显示匹配的分类标题
			$(".duty-item-name").removeClass("hidden");
		},
		onEmpty(el) {
			// 清空搜索时显示所有分类标题和容器
			$(".duty-item-name").removeClass("hidden");
			$('.duty-item').removeClass("hidden");
		}
	});

	// 检查并隐藏没有可见链接的分类容器
	function checkAndHideEmptyCategories() {
		$('.duty-item').each(function() {
			var $container = $(this);
			var $visibleLinks = $container.find('.duty-card-item:not(.hidden)');
			
			// 如果该分类下没有可见的链接，隐藏整个分类容器
